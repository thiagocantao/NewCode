...
props.field?.name || 
(props.field?.title && typeof props.field.title === 'object' ? props.field.title[currentLang.value] : props.field.title) || 
'Unnamed Field';
});
...
// Create a unique selector that won't conflict with other instances
const uniqueId = `field-${fieldId}-${sectionFieldId}-${Date.now()}`;
const selector = `[data-field-id="${fieldId}"][data-section-field-id="${sectionFieldId}"]`;
