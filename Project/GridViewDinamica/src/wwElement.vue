...
import UserCellRenderer from "./components/ResponsibleUserCellRenderer.js";
...
class ListCellEditor {
  init(params) {
    ...
    } else if (typeof params.colDef.listOptions === 'string' && params.colDef.listOptions.trim() !== '') {
      optionsArr = params.colDef.listOptions.split(',').map(o => o.trim());
    } else if (params.colDef.dataSource && typeof params.colDef.dataSource.list_options === 'string' && params.colDef.dataSource.list_options.trim() !== '') {
      optionsArr = params.colDef.dataSource.list_options.split(',').map(o => o.trim());
    }
  ...
  formatOption(opt) {
    ...
    try {
      if (params.useCustomFormatter && typeof params.formatter === 'string') {
        const fn = new Function('value', 'row', 'colDef', 'getRoundedSpanColor', 'dateFormatter', params.formatter);
        ...
        const styled = this.getRoundedSpanColor(value, params.styleArray, colDef.context?.FieldDB);
...
export default {
  ...
  setup(props, ctx) {
    ...
    const onGridReady = (params) => {
      ...
      setTimeout(() => applyPinnedHeaderBlock(), 500);

      // Seleção sempre primeira coluna
      if (props.content.rowSelection === 'multiple' && !props.content.disableCheckboxes) {
        ...
        setTimeout(() => {
          ...
          if (gridElement) {
            ...
            observer.observe(gridElement, { childList: true, subtree: true, attributes: false });
    ...
    function restorePinnedColumns() {
      ...
      const others = [];
      state.forEach(col => {
        const original = props.content && props.content.columns
          ? props.content.columns.find(c => c.id === col.colId || c.field === col.colId)
          : null;
    ...
    const forceSelectionColumnFirst = () => {
      ...
      try {
        ...
        const selectionColumnIndex = columnState.findIndex(col => col.colId === 'ag-Grid-SelectionColumn');
    ...
    // Atualiza rowData explicitamente quando a fonte muda (robusto para publish)
    watch(() => props.content?.rowData, () => {
      if (gridApi.value) {
        const rows = normalizeCollectionData(props.content?.rowData);
        try {
          gridApi.value.setRowData(rows);
        } catch (_) {
          gridApi.value.refreshClientSideRowModel?.('everything');
        }
      }
    }, { deep: true });
  ...
  computed: {
    ...
    finalColumnDefs() {
      ...
      if (this.content.rowSelection === 'multiple' && !this.content.disableCheckboxes) {
        return columns.map(col => {
          const orig = col.context?._originalPinned;
          if (orig) {
            return { ...col, pinned: orig };
          }
          return col;
        });
      }
    ...
    columnDefs() {
      ...
      return orderedColumns.map((colCopy) => {
        const toNumber = (val) => {
        ...
        const userData = {
          FieldDB: colCopy.FieldDB || null,
          TagControl: tagControl || null,
          _originalPinned: colCopy.pinned === 'left' ? 'left' : (colCopy.pinned === 'right' ? 'right' : null),
        };

        const commonProperties = {
          colId: colCopy.id || colCopy.field,        // usar colId, não "id"
          field: colCopy.field,
          minWidth,
          ...(width ? { width } : {}),
          ...(isFlex ? { flex } : {}),
            ...
            cellRenderer: (params) => {
              ...
              function getDeadlineDiff(val) {
                ...
                if (abs < 60 * 1000) {
                  ...
                  const m = Math.floor((abs % (60 * 60 * 1000)) / (60 * 1000));
                  return `${isPast ? '-' : ''}${d}d${h > 0 ? ` ${h}h` : ''}${m > 0 ? ` ${m}m` : ''}`;
                }
              }
        ...
        // Filtro de lista com renderers/editores
        if (colCopy.filter === 'agListColumnFilter') {
          ...
          const result = {
            ...
            cellRenderer: isResponsible ? 'ResponsibleUserCellRenderer' : 'FormatterCellRenderer',
            cellRendererParams: {
          ...
          if (isResponsible) {
            if (colCopy.editable) {
              ...
              result.cellEditorParams = optionsArr && optionsArr.length
                ? { options: optionsArr }
                : (params => ({ options: getDsOptions(params) }));
            }
          ...
          if (colCopy.cellDataType === 'list' || tagControl === 'LIST') {
            if (colCopy.editable) {
              result.editable = true;
          ...
          if (colCopy.dataSource && colCopy.editable) {
            result.editable = true;
            result.cellEditor = isResponsible ? ResponsibleUserCellEditor : FixedListCellEditor;
            result.cellEditorParams = params => ({ options: getDsOptions(params) });
            const baseRendererParams = result.cellRendererParams;
            result.cellRendererParams = params => ({
              ...(typeof baseRendererParams === 'function' ? baseRendererParams(params) : baseRendererParams),
        ...
        switch (colCopy.cellDataType) {
          ...
          case "custom":
            return {
              ...
              cellRendererParams: { containerId: colCopy.containerId },
              sortable: colCopy.sortable,
          ...
          case "image":
            return {
              ...
              cellRendererParams: { width: colCopy.imageWidth, height: colCopy.imageHeight },
            };
          case "list": {
            ...
            const result = {
              ...
              editable: !!colCopy.editable,
              cellEditor: staticOptions && staticOptions.length
                ? ListCellEditor
            ...
            if (staticOptions && staticOptions.length) {
              ...
              result.cellRendererParams = { ...result.cellRendererParams, options: staticOptions };
            } else {
              result.cellEditorParams = params => ({ options: getDsOptions(params) });
              const baseRendererParams = result.cellRendererParams;
              result.cellRendererParams = params => ({
            ...
            if (colCopy.textAlign) baseCellStyle = params => ({ textAlign: colCopy.textAlign });
            if (colCopy.cursor) {
              const prev = baseCellStyle;
              baseCellStyle = params => ({ ...(typeof prev === 'function' ? prev(params) : {}), cursor: colCopy.cursor });
            } else if (result.editable) {
              const prev = baseCellStyle;
              baseCellStyle = params => ({ ...(typeof prev === 'function' ? prev(params) : {}), cursor: 'pointer' });
            } else if (colCopy.FieldDB === 'TicketNumber') {
              const prev = baseCellStyle;
              baseCellStyle = params => ({ ...(typeof prev === 'function' ? prev(params) : {}), cursor: 'pointer' });
            }
            if (baseCellStyle) result.cellStyle = baseCellStyle;
            return result;
          }
          default: {
            const result = {
            ...
            if (colCopy.useCustomFormatter) {
              ...
              result.cellRendererParams = { useCustomFormatter: true, formatter: colCopy.formatter };
            } else if (colCopy.useStyleArray) {
              result.cellRenderer = tagControl === 'RESPONSIBLEUSERID' ? "ResponsibleUserCellRenderer" : 'FormatterCellRenderer';
              result.cellRendererParams = { useCustomFormatter: false, useStyleArray: true, styleArray: this.content.cellStyleArray };
            }
            ...
            if (tagControl === 'DATE' || colCopy.cellDataType === 'date' || colCopy.cellDataType === 'dateString') {
              ...
              if (colCopy.editable) result.cellEditor = DateTimeCellEditor;
            ...
            if (colCopy.cellDataType === 'list' || tagControl === 'LIST') {
              ...
              if (colCopy.editable) {
                ...
                if (optionsArr && optionsArr.length) {
                  result.cellEditor = ListCellEditor;
                  result.cellEditorParams = { options: optionsArr };
                  result.cellRendererParams = { ...(result.cellRendererParams || {}), options: optionsArr };
                } else {
  ...
  methods: {
    ...
    setFilters(filters) {
      if (this.gridApi) this.gridApi.setFilterModel(filters || null);
    },
    getRowId(params) {
      try {
        const { resolveMappingFormula } = wwLib.wwFormula.useFormula();
        const maybe = resolveMappingFormula?.(this.content.idFormula, params.data);
        if (maybe !== undefined && maybe !== null && maybe !== '') return String(maybe);
      } catch (_) {}
      return String(params.data?.ID ?? params.data?.Id ?? params.data?.id ?? params.rowIndex);
    },
    ...
    onCellValueChanged(event) {
      ...
      const tag = (colDef.context?.TagControl || '').toUpperCase();
      const identifier = (colDef.context?.FieldDB || '').toUpperCase();
      ...
      if (tag === 'RESPONSIBLEUSERID' || identifier === 'RESPONSIBLEUSERID') {
        ...
        if (match) {
          ...
          if (this.gridApi && event.node) {
            this.gridApi.refreshCells({ rowNodes: [event.node], columns: [fieldKey], force: true });
          }
        }
      }
    ...
    onRowClicked(event) {
      ...
      if (colId) {
        ...
        if (colConfig) fieldDB = colConfig.FieldDB;
      }
      this.$emit("trigger-event", {
        name: "rowClicked",
        event: {
    ...
    onCellClicked(event) {
      ...
      let fieldID = null;
      if (colId) {
        const colConfig = this.content.columns.find(col => col.id == colId || col.field == colId || col.colId == colId);
        if (colConfig) {
    ...
    isColumnMovable(params) {
      ...
      if (colDef && (colDef.pinned === 'left' || colDef.pinned === 'right')) return false;
  ...
  watch: {
        ...
        if (newValue !== oldValue && this.gridApi) {
          this.$nextTick(() => {
            if (newValue === 'multiple' && !this.content.disableCheckboxes) {
              setTimeout(() => { this.forceSelectionColumnFirst(); }, 100);
              setTimeout(() => { this.forceSelectionColumnFirstDOM(); }, 300);
            }
          });
        }
      },
    ...
    'content.selectAllRows'(newValue) {
      ...
      if (this.gridApi) {
        if (newValue === 'S') this.gridApi.selectAll();
        else if (newValue === 'N') this.gridApi.deselectAll();
      }
    },
  },
  /* wwEditor:end */
...
.ww-datagrid {
  ...
  :deep(.ag-header-cell[col-id="ag-Grid-SelectionColumn"]) {
    ...
    .ag-header-cell-sortable { cursor: default !important; }
  ...
  :deep(.ag-header-cell.ag-pinned-left):not([col-id="ag-Grid-SelectionColumn"]) { order: -1 !important; }
  :deep(.ag-row .ag-cell.ag-pinned-left):not([col-id="ag-Grid-SelectionColumn"]) { order: -1 !important; }
  ...
  :deep(.ag-row) { border: none !important; }
  :deep(.ag-row:last-child .ag-cell) { border-bottom: none !important; }
...
:deep(.ag-header-align-left .ag-header-cell-label) { justify-content: flex-start !important; }
...
:deep(.ag-header-align-right .ag-header-cell-label) { justify-content: flex-end !important; }
...
/* Paginação com fonte igual às linhas */
...
:deep(.deadline-green) { background: #9bbff6 !important; color: #fff !important; }
:deep(.deadline-yellow) { background: #ffd54f !important; color: #fff !important; }
:deep(.deadline-red) { background: #ff6f6f !important; color: #fff !important; }
</style>
